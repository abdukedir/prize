...
export default function EvenOddGamePage() {
  ...
  const profitPreview = selectedAmount * (1 - houseRate);
  ...
  async function placeBet() {
    ...
    if (participant.balance < selectedAmount) {
      return toast.error(`${participant.name} needs ${money(balanceShortfall)} more to play ${formatK(selectedAmount)}`);
    }
    ...
    try {
      ...
      if (matchingRoom) {
        ...
        if (selectedAmount > matchingRoom.remaining) return toast.error(`Only ${formatK(matchingRoom.remaining)} remains to match this room`);
        await api(`/api/even-odd/rooms/${matchingRoom.id}/join`, { method: "POST", body: JSON.stringify({ participantId: selectedParticipantId, amount: selectedAmount }) });
        toast.success(selectedAmount === matchingRoom.remaining ? "Room matched" : "Bet added to matching side");
      } else {
        await api("/api/even-odd", { method: "POST", body: JSON.stringify({ participantId: selectedParticipantId, side: selectedSide, amount: selectedAmount }) });
        toast.success(`${formatK(selectedAmount)} ${selectedSide.toLowerCase()} bet opened`);
      }
      await load(true, selectedParticipantRef.current);
    } catch (error) {
      ...
      <div className="space-y-2">
        ...
        <section className="grid items-stretch gap-2 grid-cols-1 lg:grid-cols-2">
          <div className="panel flex h-full flex-col gap-2 p-2 min-h-[300px] text-[10px]">
            ...
            <div className="flex items-center justify-between rounded border border-zinc-100 bg-white/70 px-2 py-1.5 text-[10px] dark:border-zinc-800 dark:bg-zinc-950/40">
              <span className="font-semibold text-zinc-500">Balance</span>
              <span className="font-bold text-zinc-900 dark:text-white">{money(participant?.balance ?? 0)}</span>
            </div>
            {participant && balanceShortfall > 0 ? (
              <div className="rounded border border-red-200 bg-red-50 px-3 py-2 text-[10px] font-semibold text-red-700 dark:border-red-900 dark:bg-red-950/30 dark:text-red-300">
                {participant.name} needs {money(balanceShortfall)} more to play {formatK(selectedAmount)}.
              </div>
            ) : null}
            ...
            <div className="rounded border border-emerald-100 bg-emerald-50 px-3 py-2 text-[10px] text-zinc-700 dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-zinc-200">
              <p className="text-[9px] font-semibold uppercase tracking-wide text-emerald-700 dark:text-emerald-300">Game Room</p>
              {matchingRoom ? (
                <div className="mt-1 grid grid-cols-2 gap-1">
                  <div className="rounded bg-white/70 px-2 py-1 dark:bg-zinc-950/40">
                    <p className="text-[9px] text-zinc-500">Room</p>
                    <p className="font-black">#{matchingRoom.roomNumber}</p>
                  </div>
                  <div className="rounded bg-white/70 px-2 py-1 dark:bg-zinc-950/40">
                    <p className="text-[9px] text-zinc-500">Remaining</p>
                    <p className="font-black">{formatK(matchingRoom.remaining)}</p>
                  </div>
                </div>
              ) : (
                <div className="mt-1 grid grid-cols-2 gap-1">
                  <div className="rounded bg-white/70 px-2 py-1 dark:bg-zinc-950/40">
                    <p className="text-[9px] text-zinc-500">New Room</p>
                    <p className="font-black">{selectedSide}</p>
                  </div>
                  <div className="rounded bg-white/70 px-2 py-1 dark:bg-zinc-950/40">
                    <p className="text-[9px] text-zinc-500">Target</p>
                    <p className="font-black">{formatK(selectedAmount)}</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <button className="h-8 w-full rounded-lg text-xs font-semibold transition-all duration-200 flex items-center justify-center gap-2 btn-primary" disabled={busy || matchedRooms.length === 0} onClick={finishGame}>
              <Play size={14} />
              Finish Matched Game
            </button>
          </div>
                        ...
                        <td className="px-1.5 sm:px-3 py-1 sm:py-2">
                          <div className="flex flex-wrap gap-0.5 sm:gap-1">
                            {selected.length > 0 ? selected.map((label) => (
                              <span key={label} className="rounded bg-emerald-50 px-1 sm:px-2 py-0.5 text-[6px] sm:text-[8px] md:text-[9px] font-bold text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">{label}</span>
